...
import { FiHash, FiCalendar, FiDollarSign, FiUser, FiLoader } from "react-icons/fi";
...
export default function AdminPayments() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-xs">Loading transaction logs...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 text-left">
      <div>
        <h1 className="text-2xl font-black text-slate-900 dark:text-white">Stripe Transaction Logs</h1>
        <p className="text-xs text-slate-500 dark:text-slate-400">View and audit all credit card consultation payments.</p>
      </div>
      ...
      {payments.length > 0 ? (
            ...
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 text-left">
              <thead className="bg-slate-50 dark:bg-slate-950">
                <tr>
                  ...
                  <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-455">Date & Time</th>
                  <th className="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-455 text-right">Amount</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-sm">
                {payments.map((pay) => (
                  <tr key={pay._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-slate-700 dark:text-slate-400">
                      {pay.patientName}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-slate-705 dark:text-slate-400">
        ...
        <p className="text-center py-20 text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-white dark:bg-slate-900">
          No transactions registered in the system database.
        </p>
      )}
    </div>
  );
